    ...
    def encode(self, videos):
        with amp.autocast(device_type="cuda", dtype=self.dtype):
            ...
            if isinstance(videos, list):
                return [self.model.encode(u.unsqueeze(0), scale).float().squeeze(0) for u in videos]
            if isinstance(videos, torch.Tensor):
                return self.model.encode(videos, scale).float()
        raise TypeError("videos should be a list or a tensor")

    def decode(self, zs):
        with amp.autocast(device_type="cuda", dtype=self.dtype):
            scale = [self.mean, 1.0 / self.std]
            if isinstance(zs, list):
                return [self.model.decode(u.unsqueeze(0), scale).float().clamp_(-1, 1).squeeze(0) for u in zs]
            if isinstance(zs, torch.Tensor):
                return self.model.decode(zs, scale).float().clamp_(-1, 1)
        raise TypeError("zs should be a list or a tensor")
